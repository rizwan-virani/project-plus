/* ============================================================================
   project+  ::  flashEngine.js  —  Rapid Recall Flashcards
   ...
   - Cards live in lazy-loaded per-domain modules (assets/js/content/flashN.js),
     each populating PROJPLUS.flash[N] with 100 cards { id, d, t, obj, front, back }.
   - Scheduling state persists to localStorage (projplus.flash.v1) keyed by card id,
...
(function () {
  ...
  function ver() { return window.PROJPLUS_VER ? "?v=" + window.PROJPLUS_VER : ""; }
  function now() { return Date.now(); }
  /* Domain ids and total card count derived from the exam metadata so the engine
     is not hardcoded to a fixed domain count (this site has fewer than the master's). */
  function domainIds() { return (S.domainMeta || []).map(function (d) { return d.id; }); }
  function totalCards() { return DECK_SIZE * domainIds().length; }
  ...
  function loadAll(cb) {
    var need = domainIds().filter(function (n) { return !(S.flash && S.flash[n] && S.flash[n].length); });
  ...
  function allCards() { var out = []; domainIds().forEach(function (n) { out = out.concat(S.flash[n] || []); }); return out; }
  ...
  window.FLASH = {
    ...
    allDue: function () {
      ...
      return totalCards() - notDue;
    },
    masterAcronymCount: function () { return (S.flash && domainIds().every(function (n) { return S.flash[n]; })) ? masterAcronyms().length : null; },
    ...
    analytics: function () {
      var st = store();
      var agg = { total: totalCards(), studied: 0, mastered: 0, learning: 0, reps: 0, lapses: 0 };
      var byD = {}, _ids = domainIds(); for (var _i = 0; _i < _ids.length; _i++) { var n = _ids[_i]; byD[n] = { total: DECK_SIZE, studied: 0, mastered: 0, due: this.domainDue(n) }; }
      for (var id in st) {
