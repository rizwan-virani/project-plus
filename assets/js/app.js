/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `PROJPLUS` namespace with
   ...
   - quizEngine.js adds the 400-question bank, the 30-object PBQ database, and
...
(function () {
  ...
  var S = window.PROJPLUS || {};
  var PASS = (S.exam && S.exam.passing) || 0;
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("projplus." + k, fb); },
      set: function (k, v) { write("projplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("projplus." + k); } catch (e) {} }
    };
  })();
  ...
  function statCard() {
    ...
    return el("div.kpis", null, [
      ...
      kpi(pbqDone + "/" + (S.pbqs ? S.pbqs.length : 0), "PBQs completed", ""),
      flash ? kpi(flash.mastered + "/" + flash.total, "Cards mastered", flash.mastered >= flash.total / 2 ? "good" : (flash.mastered ? "warn" : "")) : null
    ]);
  }
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the " + ((S.exam && S.exam.code) || "") + " is scored and delivered, and where the credential fits in a project-management career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the " + ((S.exam && S.exam.code) || "") + " &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where Project+ sits on the career ladder: a lifetime, no-renewal credential, and the project coordinator, junior PM, and PMO analyst roles it unlocks.",
      foot: "Foundational · lifetime certification", cta: "Read",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every Project+ acronym across all " + nDomains + " domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "400+") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all " + nDomains + " domains." }));
    ...
    g3b.appendChild(card({
      ...
      desc: "Ten questions drawn uniformly from all " + nDomains + " domains &mdash; a fast, mixed knowledge check with full rationale on every item.",
    ...
    var b6 = dashSection("06", "Performance-based question simulators", (S.pbqs ? S.pbqs.length : 0) + " simulations · " + ((S.pbqFormats ? S.pbqFormats.length : 0)) + " formats");
    b6.appendChild(el("p.sectsub", { html: "A database of <b>" + (S.pbqs ? S.pbqs.length : 0) + "</b> hands-on PBQs across " + ((S.pbqFormats ? S.pbqFormats.length : 0)) + " formats. Read the exhibit, configure each field, and submit for graded feedback mapped to the objective." }));
    var g4 = el("div.grid");
    ...
    bLabs.appendChild(el("p.sectsub", { html: "Scenario-driven, hands-on laboratory exercises. Flip a card to review its learning objectives, then launch a full-screen interactive sandbox console with a live terminal log." }));
    renderLabsInline(bLabs);

    /* ---- 08 · Mock Certification Exam ---- */
    var examMax = (S.exam && S.exam.maxQuestions) || "";
    var examMin = (S.exam && S.exam.minutes) || "";
    var b5 = dashSection("08", "Mock certification exam", examMax + " questions · timed");
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the " + ((S.exam && S.exam.code) || "") + " &mdash; " + examMax + " questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    g3c.appendChild(card({
      icon: "🧪", tag: "MOCK EXAM", title: "Full-Length Mock Exam",
      desc: "A realistic " + examMin + "-minute simulation: " + examMax + " questions weighted to the exact domain percentages, with a countdown timer, question flagging, and a domain-by-domain scoring dashboard.",
      foot: examMax + " questions · " + examMin + " minutes · timed", cta: "Begin",
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: the official CompTIA Project+ objectives PDF, the PMI Lexicon and EVM references, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.PROJPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. Project+" });
  ...
  function computeReadiness() {
    ...
    var dims = [
      ...
      { key: "mock", label: "Mock exam", weight: 1.6, value: bestMock != null ? Math.round(bestMock / S.exam.scaleHigh * 100) : null, has: mocks.length > 0, kind: "score", detail: bestMock != null ? ("best " + bestMock + "/" + S.exam.scaleHigh) : "not attempted" },
  ...
  function readinessOutcome(R) {
    var ceil = (S.exam && S.exam.scaleHigh) || "";
    if (R.bestMock != null) {
      var m = R.bestMock;
      if (m >= PASS + 50) return { cls: "good", text: "Strong pass projected. Your best mock of " + m + " out of " + ceil + " clears the " + PASS + " passing line with margin." };
      if (m >= PASS) return { cls: "good", text: "On track to pass. Your best mock of " + m + " out of " + ceil + " is at or above the " + PASS + " passing line. Keep it consistent." };
      if (m >= PASS - 70) return { cls: "warn", text: "Borderline. Your best mock of " + m + " out of " + ceil + " sits just under the " + PASS + " line. A focused push should get you there." };
      return { cls: "crit", text: "Not yet. Your best mock of " + m + " out of " + ceil + " is below passing. Keep building before you book the exam." };
    }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates PROJPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.PROJPLUS_VER ? "?v=" + window.PROJPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.open(readerView("Domain " + id, d.title, ((S.exam && S.exam.code) || "") + " Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: " + ((S.exam && S.exam.code) || ""), "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where Project+ fits in your path", S.careerGuidance || [])); }
      ...
      tag: "Analytics", render: function (host) {
        ...
        h.slice(0, 60).forEach(function (r) {
          ...
          var scoreCell = r.mode === "mock"
            ? el("span.pillscore." + (pass ? "pass" : "fail"), { text: r.scaled + " / " + ((S.exam && S.exam.scaleHigh) || "") })
  ...
  /* ===========================================================================
     BOOT
     =========================================================================== */
  /* Populate the hero exam-details line from the single PROJPLUS.exam source so
     no exam figure (duration, item count, passing, scale, domains) is duplicated
     as a literal in the markup. */
  function fillExamLine() {
    var host = $("#examline"); if (!host || !S.exam) return;
    var e = S.exam;
    host.innerHTML =
      "<span>⏱ <b>" + e.minutes + " minutes</b></span>" +
      "<span>❓ <b>up to " + e.maxQuestions + " questions</b></span>" +
      "<span>🎯 <b>passing " + e.passing + "</b> / " + e.scaleHigh + "</span>" +
      "<span>📚 <b>" + e.domains + " domains</b></span>";
  }

  function boot() {
    Theme.build();
    fillExamLine();
    $("#homeBrand").addEventListener("click", function (e) { e.preventDefault(); Router.showDashboard(); fillExamLine(); renderDashboard(); });
    renderDashboard();
